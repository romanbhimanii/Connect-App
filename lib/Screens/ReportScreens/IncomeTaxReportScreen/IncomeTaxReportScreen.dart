      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(text: 'LONGTERM'),
            Tab(text: 'OP ASSETS'),
            Tab(text: 'OP SHORTTERM'),
            Tab(text: 'SHORTTERM'),
            Tab(text: 'TRADING'),
            Tab(text: 'GRAND TOTAL'),
          ],
        ),
      ),
      body: TabBarView(
          ...
          child: Stack(
            children: [
              Column(
                children: [
                  Expanded(
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: (report.data?.aSSETS?.length ?? 0) - 1,
                                              ...
                                              Column(
                                                children: [
                                                  ...
                                                  Utils.text(
                                                    ...
                                                    color: (assetsReport?.netAmount?.startsWith('N/A') ?? false) ? Colors.red : Colors.green,
                                                    fontSize: 12,
                                                    textOverFlow: TextOverflow.ellipsis,
                                                    fontWeight: FontWeight.w600
                                                  ),
                    ...
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: (report.data?.eXPENSES?.length ?? 0) - 1,
                            physics: const BouncingScrollPhysics(),
                            padding: const EdgeInsets.all(0),
                            itemBuilder: (context, index) {
                          ...
                          //         DataColumn(label: Text('Scrip Name')),
                          //         DataColumn(label: Text('Net Qty')),
                          //         DataColumn(label: Text('Net Rate')),
                          //         DataColumn(label: Text('Net Amount')),
                          //         DataColumn(label: Text('Closing Price')),
                          //         DataColumn(label: Text('PL Amt')),
                          //         DataColumn(label: Text('Buy Qty')),
                          //         DataColumn(label: Text('Buy Rate')),
                          //         DataColumn(label: Text('Buy Amt')),
                          //         DataColumn(label: Text('Sale Qty')),
                          //       ],
                          //       rows: List<DataRow>.generate(report.data!.eXPENSES!.length, (index) {
                          //         final item = report.data!.eXPENSES![index];
                          //         return DataRow(
                          //           cells: [
                          //             DataCell(Text((index + 1).toString())),
                          //             DataCell(Text(item.trType ?? 'N/A')),
                          //             DataCell(Text(item.netQty?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.netRate?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.netAmount?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.closingPrice?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.plAmt?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.buyQty?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.buyRate?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.buyAmt?.toString() ?? 'N/A')),
                          //             DataCell(Text(item.saleQty?.toString() ?? 'N/A')),
                          //           ],
                          //         );
                          //       }),
                          //     ),
                    ...
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: (report.data?.lONGTERM?.length ?? 0) - 1,
                            physics: const BouncingScrollPhysics(),
                            padding: const EdgeInsets.all(0),
                            itemBuilder: (context, index) {
                    ...
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: (report.data?.oPASSETS?.length ?? 0) - 1,
                            physics: const BouncingScrollPhysics(),
                            padding: const EdgeInsets.all(0),
                            itemBuilder: (context, index) {
                    ...
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: (report.data?.oPSHORTTERM?.length ?? 0) - 1,
                            physics: const BouncingScrollPhysics(),
                            padding: const EdgeInsets.all(0),
                            itemBuilder: (context, index) {
                    ...
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: (report.data?.sHORTTERM?.length ?? 0) - 1,
                            physics: const BouncingScrollPhysics(),
                            padding: const EdgeInsets.all(0),
                            itemBuilder: (context, index) {
                    ...
                    child: FutureBuilder<IncomeTaxReport?>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return ListView.builder(
                            shrinkWrap: true,
                            itemCount: report.data?.tRADING?.length,
                            physics: const BouncingScrollPhysics(),
                            padding: const EdgeInsets.all(0),
                            itemBuilder: (context, index) {
